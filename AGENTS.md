...
- Manager / CLI 实例参数 `run_duration`、`pause_duration` 同时填写才启用，单位分钟，格式为 `基础值[;随机增量]`；每段运行及每次暂停重新随机。
- `autogui.runtime.schedule.RunPauseSchedule` 由 `RuntimeContext` 实例持有；首次进入主流程开始计时，子流程、脚本、等待、recovery 均累计运行时间，恢复成功也不重置计时器。
- 仅在最外层 `main.csv` 第一条节点的 `AutoOperator.Update()` 入口执行暂停检查。正常循环、跳转回第一条、第一条重试、recovery 后重启均算；子流程（即使名为 `main.csv`）和 recovery 本身不暂停。
- 到期后不打断当前轮次，等待回到入口；一直不回到入口则不暂停。配置作者必须保证入口状态可以长时间停留并重新开始，用户文档必须明确这个限制。
- 暂停使用可响应停止事件的等待。watchdog 使用计时器扣除实际暂停时间的时钟，保留已有无进展时长和次数；停止期间不得误触发 recovery。
- Manager 暂停/恢复状态经现有队列由主线程应用，暂停实例仍是存活实例，可停止或重启。加载旧参数文件必须清空新增时长字段。
