...
- `exist;A;B` 和 `notExist;A;B` 是双跳转；`A/B` 可以是当前流程中的有效序号、跳转标记或负整数。
- 没有分支参数的 `pic/ocr` 会持续重试直到命中，然后把鼠标移动到目标中心。
- `jmp` 可以指向当前流程中存在的序号、跳转标记或负整数。
- 任一跳转目标为负整数时，运行时立即结束当前 `AutoOperator`，不执行后续节点或等待时间；子流程会返回父流程，主流程会结束当前会话。推荐使用 `-1`，其他负整数语义相同。
