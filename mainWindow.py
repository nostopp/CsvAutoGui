...
class MainWindow:
    def __init__(self, root:tk.Tk):
        ...
        ttk.Label(frm_top, text='config').grid(row=0, column=0, sticky='w')
        self.e_config = ttk.Entry(frm_top, width=40)
        ...
        ttk.Checkbutton(frm_top, text='loop', variable=self.var_loop).grid(row=1, column=0)

        self.var_log = tk.BooleanVar(value=defaults.log)
        ttk.Checkbutton(frm_top, text='log', variable=self.var_log).grid(row=1, column=1)

        ttk.Label(frm_top, text='scale').grid(row=2, column=0)
        self.e_scale = ttk.Entry(frm_top, width=4)
        self.e_scale.insert(0, str(defaults.scale))
        self.e_scale.grid(row=2, column=1)

        ttk.Label(frm_top, text='offset').grid(row=2, column=2)
        self.e_offset = ttk.Entry(frm_top, width=4)
        ...
        ttk.Checkbutton(frm_top, text='scale_image', variable=self.var_scale_image).grid(row=2, column=4)

        ttk.Label(frm_top, text='title').grid(row=3, column=0)
        self.e_title = ttk.Entry(frm_top, width=40)
        if defaults.title:
        ...
        ttk.Checkbutton(frm_top, text='screenshots', variable=self.var_screenshots).grid(row=4, column=1)

        self.var_record = tk.BooleanVar(value=defaults.record)
        ttk.Checkbutton(frm_top, text='record', variable=self.var_record).grid(row=4, column=2)

        btn_start = ttk.Button(frm_top, text='Start Instance', command=self.start_instance)
        btn_start.grid(row=5, column=0, pady=6)
        Tooltip(btn_start, "点击启动一个新的实例(以当前参数)")

        btn_stop = ttk.Button(frm_top, text='Stop Selected', command=self.stop_selected)
        btn_stop.grid(row=5, column=1)
        Tooltip(btn_stop, "停止所选实例")

        btn_stop_all = ttk.Button(frm_top, text='Stop All', command=self.stop_all)
        btn_stop_all.grid(row=5, column=2)
        Tooltip(btn_stop_all, "停止所有实例")

        btn_restart = ttk.Button(frm_top, text='Restart Selected', command=self.restart_selected)
        ...
        list_scroll = ttk.Scrollbar(frm_mid, orient=tk.VERTICAL)
        self.treeview = ttk.Treeview(frm_mid, columns=('instance',), show='headings', 
                                      yscrollcommand=list_scroll.set, selectmode='browse')
        list_scroll.config(command=self.treeview.yview)
        self.treeview.pack(side=tk.LEFT, fill=tk.Y)
        list_scroll.pack(side=tk.LEFT, fill=tk.Y)
        
        self.treeview.heading('instance', text='实例列表')
        self.treeview.column('instance', width=180, stretch=False, anchor='w')
        self.treeview.bind('<<TreeviewSelect>>', self.on_select)

        self.txt_log = scrolledtext.ScrolledText(frm_mid, state='disabled', width=70)
        self.txt_log.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        frm_bottom = ttk.Frame(root)
        frm_bottom.pack(fill=tk.X, padx=6, pady=6)

        btn_clear = ttk.Button(frm_bottom, text='Clear Instances', command=self.clear_instances)
        btn_clear.pack(side=tk.LEFT)
        Tooltip(btn_clear, "清空所有实例(停止并移除)")

        btn_reload_csv = ttk.Button(frm_bottom, text='Reload CSV', command=self.reload_csv)
        btn_reload_csv.pack(side=tk.LEFT, padx=6)
        Tooltip(btn_reload_csv, "重新加载所有 CSV 文件")

        btn_save = ttk.Button(frm_bottom, text='Save Params', command=self.save_params)
        btn_save.pack(side=tk.LEFT, padx=6)
        Tooltip(btn_save, "保存当前参数到 JSON 文件")

        btn_load = ttk.Button(frm_bottom, text='Load Params', command=self.load_params)
        btn_load.pack(side=tk.LEFT)
        Tooltip(btn_load, "从 JSON 文件加载参数")

        # 快捷键说明
        lbl_hotkey = ttk.Label(frm_bottom, text='快捷键 Shift+Ctrl+X：停止所有实例', style='Blue.TLabel')
            ...
            def _mark_running():
                try:
                    ...
                    if self.treeview.exists(item_id):
                        self.treeview.item(item_id, values=(f"{inst.name} (id:{inst.id}) [running]",))
                except Exception:
                    pass
            ...
            try:
                ...
                def _mark_stopped():
                    try:
                        item_id = f"item_{inst.id}"
                        if self.treeview.exists(item_id):
                            self.treeview.item(item_id, values=(f"{inst.name} (id:{inst.id}) [stopped]",))
                    except Exception:
                        pass
    ...
    def _start_instance_with_args(self, args):
        ...
        item_id = f"item_{iid}"
        self.treeview.insert('', tk.END, iid=item_id, values=(display_name,))
    ...
    def get_selected_instance(self):
        ...
        try:
            item_id = sel[0]
            # 从 item_id 中提取实例 ID (格式: "item_123")
            iid = int(item_id.split('_')[1])
