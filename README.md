...
| --run_duration | 运行时长（分钟），格式为 `基础值` 或 `基础值;随机增量`，需与暂停时长同时设置 | 留空 |
| --pause_duration | 暂停时长（分钟），格式同上，仅在最外层 `main.csv` 第一条节点前暂停 | 留空 |
...
Manager 的“运行参数”区域提供“运行时长（分钟）”和“暂停时长（分钟）”，可以随“保存参数 / 加载参数”保存和恢复。它们是每次启动实例的参数，不写入流程 CSV 或配置目录中的 `runtime.json`；同一配置的不同实例可以使用不同节奏。加载不含这两个字段的旧参数文件时，两项会清空。

| 运行时长 | 暂停时长 | 效果 |
| --- | --- | --- |
| `10` | `5` | 运行至少 10 分钟，回到允许暂停的入口后暂停 5 分钟 |
| `10;10` | `5;2` | 每段运行时长为 10 加 0～10 随机分钟，每次暂停为 5 加 0～2 随机分钟 |
| `0.5` | `0.2;0.1` | 支持小数分钟，随机增量按连续均匀分布抽取 |

两项必须同时填写才启用。

**暂停只发生在即将执行最外层 `main.csv` 第一条节点的位置。运行时长是触发暂停的最低运行时间，不是强制中断时间。** 到期时仍在本轮流程中，则继续执行，直到再次准备执行 `main.csv` 第一条节点才暂停。因此实际连续运行时间可能超过设定值；如果一直不返回该入口，就不会进入计划暂停。

- 未开启循环且流程正常结束时，直接退出，不追加暂停或重新运行。截图模式与进程获取模式不使用计划暂停。
- 暂停期间 Manager 显示“已暂停”，恢复后显示“运行中”；日志记录运行时长、暂停时长和恢复事件。停止按钮及 `Ctrl+Shift+X` 可以立即中断计划暂停。

CLI 示例（PowerShell 中分号参数需要引号）：

```powershell
uv run main.py -c config/你的配置 -l --run_duration "10;10" --pause_duration "5;2"
```
